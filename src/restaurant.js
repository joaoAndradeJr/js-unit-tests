...
const createMenu = (myMenu) => {
  const novoObjeto = {
    fetchMenu: myMenu,
    consumption: [],
    orderFromMenu: function(item) {
      novoObjeto.consumption.push(item);
    },
    pay: function() {
      let sum = 0;
      novoObjeto.consumption.forEach((item) => {
        sum += novoObjeto.fetchMenu[item];
        return sum;
      });
...
//   createMenu().novoObjeto.consumption.push(item);
// }
