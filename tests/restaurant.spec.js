...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    let menu = createMenu({food: {'coxinha': 3.90, 'sanduiche': 9.90},
    drinks: {'agua': 3.90, 'cerveja': 6.90}});
    assert.deepStrictEqual(Object.entries(menu)[0], ['fetchMenu', {food: {'coxinha': 3.90, 'sanduiche': 9.90},
    drinks: {'agua': 3.90, 'cerveja': 6.90}}]);
    ...
    assert.deepStrictEqual(Object.keys(menu.fetchMenu), [ 'food', 'drinks' ]);
    // Agora faça o TESTE 3 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    assert.deepStrictEqual(menu.fetchMenu, {food: {'coxinha': 3.90, 'sanduiche': 9.90},
    drinks: {'agua': 3.90, 'cerveja': 6.90}});
    ...
    menu.orderFromMenu('coxinha');
    assert.deepStrictEqual(menu.consumption, ['coxinha']);
    // Agora faça o PASSO 3 no arquivo `src/restaurant.js`.
    // --------------------------------------------------------------------------------------
    // TESTE 6: Verifique que as três orders seguintes, de bebidas e comidas mescladas, somam três itens no array `objetoRetornado.consumption` conforme os itens pedidos.
    menu.orderFromMenu("agua");
    menu.orderFromMenu("sopa");
    menu.orderFromMenu("sashimi");
    ...
    menu = createMenu({'coxinha': 3.90, 'sanduiche': 9.90, 'agua': 3.90, 'cerveja': 6.90});
    menu.orderFromMenu('coxinha');
    menu.orderFromMenu('agua');
    menu.orderFromMenu('coxinha');
